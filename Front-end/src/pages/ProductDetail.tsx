            ...
            {(() => {
                ...
                <div className="lg:hidden mt-6 space-y-6">
                  <div>
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2">
                      {currentProduct.category?.name ?? 'Cossack'}
                    </p>
                    <h1 className="text-[#0B0B0B] font-black text-2xl italic uppercase tracking-tighter mb-3">
                      {currentProduct.name}
                    </h1>
                    <p className={`text-xs font-black uppercase tracking-widest ${stockInfo.className}`}>
                      {stockInfo.label}
                      {stockStatus !== 'out_of_stock' && (
                  ...
                  {currentProduct.description && (
                    <p className="text-zinc-600 text-sm leading-relaxed">{currentProduct.description}</p>
                  )}

                  {uniqueColors.length > 0 && (
                    <div>
                      <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-3">Color</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {config.options.map((option) => {
                          ...
                          return (
                            ...
                            >
                              {option.value}
                                    {mod !== 0 && <span className="ml-1 opacity-80">${formatModifier(mod)}</span>}
                                  </button>
                                );
                              })}
                            </div>
                        ...
                        {currentProduct.static_configs.length > 0 && (
                          <div className="space-y-2">
                            <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Specifications</p>
                            <dl className="grid grid-cols-1 gap-2">
                              {currentProduct.static_configs.map((cfg) => (
                                <div key={cfg.id} className="px-3 py-2 bg-white border border-zinc-100 rounded-sm text-xs">
                        ...
                        <div className="p-4 bg-white border border-zinc-100 rounded-sm space-y-2">
                          ...
                          {selectedOptions.map((opt) => (
                            <div key={opt.optionId} className="flex justify-between text-sm">
                              ...
                              <span className="font-bold text-[#0B0B0B]">${formatModifier(opt.priceModifier)}</span>
                            </div>
                          ))}
                          <div className="flex justify-between pt-2 border-t border-zinc-100">
                            <span className="font-black text-[#0B0B0B] uppercase text-xs tracking-widest">Unit Price</span>
                            <span className="font-black text-xl text-[#0B0B0B]">${priceBreakdown.unitPrice.toFixed(2)}</span>
                          </div>
                          {quantity > 1 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-zinc-500">Total ({quantity} items)</span>
                              <span className="font-black text-[#0B0B0B]">${(priceBreakdown.unitPrice * quantity).toFixed(2)}</span>
                            </div>
                          )}
                        </div>
              ...
              const desktopDetails = (
                <div className="hidden lg:block">
                  ...
                  {currentProduct.description && (
                    <p className="text-zinc-600 text-sm leading-relaxed mb-6">{currentProduct.description}</p>
                  )}
